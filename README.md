...
Программа реализует основные функции библиотеки string.h

Для компиляции библиотеки необходимо применить команду ```make```

Для запуска тестов - ```make test```

Для вывода html страницы с покрытием кода тестами - ```make gcov_report```
